    ...
    mutate(
      { postId, ...data },
      {
        onSuccess: (res) => {
          ...
          reset()
          // ✅ Toast de éxito
          toast.success('¡Comentario aceptado!', 'Tu comentario fue enviado exitosamente.')
        },
        onError: () => toast.error('Error', 'No se pudo enviar el comentario. Inténtalo de nuevo.'),
      }
    )
  }
...
function CommentCard({ comment }) {
  ...
  const avatarColors = [
    ...
    'bg-amber-800 text-amber-200', 'bg-cyan-800 text-cyan-200',
  ]
  const avatarColor = avatarColors[comment.name.charCodeAt(0) % avatarColors.length]
