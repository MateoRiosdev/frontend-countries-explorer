...
const commentSchema = z.object({
  ...
  body: z.string().min(10, 'El comentario debe tener al menos 10 caracteres').max(500, 'Máximo 500 caracteres'),
})
...
export default function CommentsSection({ countryCode, countryName }) {
  ...
  // Comentarios nuevos persistentes en localStorage
  const [localComments, setLocalComments] = useState([])

  // Cargar comentarios guardados en localStorage al montar
  useEffect(() => {
    const saved = localStorage.getItem(`comments-${countryCode}`)
    if (saved) setLocalComments(JSON.parse(saved))
  }, [countryCode])

  // Combinar comentarios: los nuevos primero, luego los de la API
      ...
      { postId, ...data },
      {
        onSuccess: (res) => {
          ...
          setLocalComments(prev => {
            ...
            localStorage.setItem(`comments-${countryCode}`, JSON.stringify(updated))
            return updated
          })
        ...
        <form onSubmit={handleSubmit(onSubmit)} noValidate className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <Field label="Nombre" error={errors.name?.message}>
              <input {...register('name')} type="text" placeholder="Tu nombre" className={inputClass(errors.name)} />
            </Field>
          ...
          <Field label="Comentario" error={errors.body?.message}>
            <textarea {...register('body')} rows={4} placeholder={`Escribe tu comentario sobre ${countryName}…`} className={inputClass(errors.body) + ' resize-none'} />
          </Field>

          <button type="submit" disabled={isPending} className="w-full sm:w-auto px-8 py-3 bg-[#d4a843] hover:bg-[#c09535] active:scale-95 text-[#04080f] font-body font-semibold text-sm rounded-xl transition-all duration-200 flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed">
...
function CommentCard({ comment }) {
  ...
  const avatarColors = ['bg-violet-800 text-violet-200','bg-sky-800 text-sky-200','bg-emerald-800 text-emerald-200','bg-rose-800 text-rose-200','bg-amber-800 text-amber-200','bg-cyan-800 text-cyan-200']
  ...
  return (
    <article className={`bg-[#0a1628] rounded-2xl p-5 border transition-all duration-300
      ${comment.isNew ? 'border-[rgba(212,168,67,0.5)] shadow-[0_0_16px_rgba(212,168,67,0.08)]' : 'border-[rgba(212,168,67,0.08)]'}`}>
      <div className="flex items-start gap-3">
        <div className={`w-9 h-9 rounded-full flex items-center justify-center text-xs font-body font-bold shrink-0 ${avatarColor}`}>{initials}</div>
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between gap-2 mb-0.5">
            <p className="text-slate-200 font-body font-medium text-sm truncate">{comment.name}</p>
            {comment.isNew && (
              <span className="text-[10px] text-[#d4a843] font-body font-medium bg-[rgba(212,168,67,0.1)] px-2 py-0.5 rounded-full border border-[rgba(212,168,67,0.2)] shrink-0">Nuevo</span>
            )}
          </div>
...
function inputClass(error) {
  return `w-full bg-[#04080f] border text-slate-200 placeholder-slate-600 font-body text-sm rounded-xl px-4 py-3
    ...
    ${error ? 'border-red-500/60 focus:border-red-500 focus:ring-red-500/20' : 'border-[rgba(212,168,67,0.2)] focus:border-[#d4a843] focus:ring-[rgba(212,168,67,0.2)]'}`
}
